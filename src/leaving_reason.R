...
duration <- function(leaving){
  leaving$duration = difftime(leaving$left_at, leaving$enrolled_at,
                                    units = c("days"))
  #leaving$duration <- as.numeric(leaving$duration, units="weeks")
  #leaving$duration <- round(leaving$duration, digits=0)
...
#create gg plot
graph1 <- ggplot(data = merged_leaving, aes(
  x = enrolled_at, y = duration, color = as.factor(run))) +
  xlab("Enrolled date") + ylab("Duration before leaving (days)") + 
  geom_point() +
...
graph2 <- ggplot(data.frame(merged_leaving), aes(x = reason_num)) +
  geom_bar() +
